; NES KALEIDOSCOPE - Psychedelic kaleidoscope visuals with techno music
; Radial symmetry patterns that pulse and morph with the beat
...
vis_twist:      .res 1      ; Twist/spiral effect
vis_flash:      .res 1      ; Flash brightness on drops
...
kal_angle:      .res 1      ; Current rotation angle
kal_sector:     .res 1      ; Which sector we're updating (0-7)
kal_radius:     .res 1      ; Current radius for radial update
kal_cx:         .res 1      ; Center X (tile coords, 0-31)
kal_cy:         .res 1      ; Center Y (tile coords, 0-29)
...
;------------------------------------------------------------------------------
update_tiles:
    ; Kaleidoscope renderer - radial symmetry!
    ; Update 8 tiles per frame (1 point mirrored to 8 positions)
    
    ldx #$02                ; 2 source points per frame
@kal_loop:
    stx vis_temp+3          ; Save outer loop counter
    
    ; Increment sector and radius for next point
    inc kal_sector
    lda kal_sector
    and #$07                ; 8 sectors
    sta kal_sector
    
    ; Radius slowly expands then resets
    lda frame_count
    and #$03                ; Every 4 frames
    bne @skip_radius
    inc kal_radius
    lda kal_radius
    cmp #$10                ; Max radius 16 (reaches edges)
    bcc @skip_radius
    lda #$01
    sta kal_radius
@skip_radius:
    
    ; Calculate tile value based on radius + angle + phase
    lda kal_radius
    clc
    adc kal_angle           ; Add rotation
    clc
    adc vis_phase           ; Add animation phase
    clc
    adc vis_pulse           ; Beat pulse
    adc vis_pulse
    sta vis_temp            ; Save tile pattern base
    
    ; Now write to all 8 mirrored positions
    ; Center is at (16, 15)
    ; For simplicity, use quadrant mirroring
    
    ; Get X/Y offset from radius and sector
    ; X and Y vary independently for full coverage
    lda kal_radius
    sta tile_x              ; X offset = radius
    
    ; Y offset cycles independently (not tied to radius!)
    ; This ensures we hit all combinations of X,Y
    lda kal_sector
    asl a
    asl a                   ; sector * 4
    clc
    adc vis_phase           ; Add animation phase for variety
    and #$0F                ; Limit to 0-15
    sta tile_y              ; Y offset independent of X
    
    ; Calculate tile value with variation
    lda vis_temp
    clc
    adc tile_x
    eor tile_y
    ...
    and #$1F
    ora #$01                ; Avoid tile 0
    sta vis_temp+2          ; Final tile value
    
    ; Write to 4 mirrored positions (4-fold symmetry)
    ; Only write if position is valid (avoid edges)
    
    ; Position 1: center + (dx, dy)
    lda #$0F                ; Center X = 15 (so radius 15 reaches X=0)
    clc
    adc tile_x
    cmp #$20                ; Off right edge?
    bcs @skip_p1
    sta vis_temp
    lda #$0F                ; Center Y = 15
    clc
    adc tile_y
    cmp #$1D                ; Max Y = 28 (avoid attributes)
    bcs @skip_p1
    sta vis_temp+1
    jsr write_tile_at
@skip_p1:
    
    ; Position 2: center - (dx, dy)
    lda #$0F
    sec
    sbc tile_x
    bmi @skip_p2            ; Off left edge?
    sta vis_temp
    lda #$0F
    sec
    sbc tile_y
    bmi @skip_p2            ; Off top edge?
    sta vis_temp+1
    jsr write_tile_at
@skip_p2:
    
    ; Position 3: center + (dx, -dy)
    lda #$0F
    clc
    adc tile_x
    cmp #$20
...
@skip_p4:
    ...
    ldx vis_temp+3          ; Restore outer loop counter
    dex
    beq @kal_done
    jmp @kal_loop           ; Loop (too far for bne)
@kal_done:
    rts

; Helper: write tile at position (vis_temp = X, vis_temp+1 = Y)
write_tile_at:
    bit PPU_STATUS
    
    ; Calculate PPU address: $2000 + Y*32 + X
    lda vis_temp+1          ; Y position
    lsr a                   ; Y / 8 for high byte contribution
    lsr a
    lsr a
    clc
    adc #$20                ; Base $2000
    ...
    lda vis_temp+2          ; Tile value
    sta PPU_DATA
    rts

; Direction tables for 8 sectors (simplified radial)
sector_dx:
    .byte $00, $01, $01, $01, $00, $FF, $FF, $FF  ; 0=up, going clockwise
sector_dy:
...
@flash_done:
    ...
    ; === KALEIDOSCOPE: Rotate angle smoothly ===
    lda frame_count
    and #$01                ; Every 2 frames
    bne @skip_angle
    inc kal_angle           ; Continuous rotation
@skip_angle:

    ; Center position wobble (slow, subtle)
    lda frame_count
    and #$3F                ; Every 64 frames
    bne @skip_center
    ; Wobble center slightly
    lda frame_count+1
    and #$03
    sta kal_cx              ; Store as small offset (0-3)
    lda frame_count+1
    lsr a
    lsr a
    and #$03
